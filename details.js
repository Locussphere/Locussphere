...
function displayModDetails() {
  ...
  let tagsHTML = `<span class="tag version">${currentMod.version}</span>`;
  ...
  document.getElementById('size').textContent = currentMod.size;
  ...
  downloadBtn.href = currentMod.downloadPath;
  downloadBtn.download = `${currentMod.title.replace(/\s+/g, '-')}.zip`;
}
...
function displayChangelog() {
  ...
  if (currentMod.changelog && currentMod.changelog.length > 0) {
    versionBadge.textContent = `Version ${currentMod.changelog[0].version}`;
    
    changelogList.innerHTML = currentMod.changelog[0].changes.map(change => `
            <li>${change}</li>
        `).join('');
  } else {
